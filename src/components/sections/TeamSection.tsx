...
const TeamSection = () => {
  ...
  return <section id="team" className="py-24 bg-secondary/30 md:py-[100px]">
      <div className="container mx-auto px-4">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto">
          {roles.map((role, index) => <div key={role.title} className="glass-card p-6 text-center group hover:border-primary/50 transition-all duration-300" style={{
          animationDelay: `${index * 100}ms`
              ...
              <div className="relative mx-auto mb-4">
                ...
                {role.required && <span className="absolute -top-1 -right-1 w-5 h-5 bg-accent rounded-full flex items-center justify-center text-[10px] font-bold text-accent-foreground">
                    ★
                  </span>}
              </div>
              <h3 className="font-bold text-lg mb-2">{role.title}</h3>
              <p className="text-sm text-muted-foreground">{role.description}</p>
              {role.required && <span className="inline-block mt-3 text-xs text-primary font-medium uppercase tracking-wider">
                  Required
                </span>}
            </div>)}
        </div>
